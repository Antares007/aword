...
Color LerpGradient(Color a, float point, float amount) { return LerpColor(LerpColor(a, BLACK, point), a, amount); }
...
void draw(step_t *steps, long count) {
  ...
  do {
    ...
    for (long i = 0; i < count; i++) {
      ...
      DrawTextEx(font, format_name(s->name), (Vector2){-ns.x / 2, -ns.y / 2}, 35, 1, fgcolor);
