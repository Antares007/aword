...
#define T(Yellow,Purple,Red,Green,Blue, zero, Navy, Lime, Maroon, Fuchsia, Olive) \
  ...
  β[--ο] = Yellow
#define GEN(Yellow)                                                     \
  N(Yellow##_goto) {                  Yellow((long)β[τ]+τ,α,  β,ο,σ); } \
...
#define B(...) (({                                              \
  T(Lime,     Fuchsia,  Maroon,     Olive,       Navy,  "bo",   \
    Navy_goto,Lime_goto,Maroon_goto,Fuchsia_goto,Olive_goto);   \
  long bτ = ο+5;                                                \
  __VA_ARGS__,                                                  \
  T(Yellow_goto,Purple_goto,Red_goto,Green_goto,Blue_goto,"bb", \
...
N(toti   ) {
  ...
  B(T(Yellow,   Purple,     Red,     Green,      Blue,      "t1", Navy,     Lime,     Maroon, Fuchsia,   Olive),
    ...
    T(Yellow,   Purple,     Red,     Green,      Blue,      "tr", Blue_ret, Green,    Red_ret,Purple_ret,Yellow_ret));
  T(Yellow_call,Purple_call,Red_call,Green_call, Blue_call, bark, Navy,     Lime,     Maroon, Fuchsia, Olive);
  grow(τ, α, β, ο, σ);
}
void ti_init(Args);
int main() {
