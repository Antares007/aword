...
N(cursor        ) { τ = β[ρYellow][1],
                    α = β[ρYellow][2], Yellow(OS); }

N(re_alfa_Blue  ) { α = β[0][1], Blue(OS); }

S(check_lr      ) { (strcmp(o[α[0][1] + 1], o[τ + 1]) == 0 ? Green : Maroon)(OS); }

S(on_name       ) { οNavy(Navy, τ), οMaroon(check_lr, "ani"), book_of_ani_s(OS); }
...
S(ani_sword     ) { οBlue(goin_sword, α), οBlue(re_alfa_Blue), οGreen(Yellow), Maroon(OS); }
