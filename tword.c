...
n_t *YGRB_separator, *left_separator, *ygrb_separator;
n_t leftleft_gateway[4];
...
N(locked_left_gate_Yellow) {
  left_separator = left_gateway;
  left_gateway[0](t, a, b, o, s);
}
N(locked_left_gate_Green) {
  left_separator = left_gateway;
...
N(locked_right_gate_Yellow) {
  YGRB_separator = right_gateway;
  o[--b] = right_gateway[0];
  o[a++] = (void *)1;
  init_next_arm(t, a, b, o, s);
}
N(locked_right_gate_Green) {
  YGRB_separator = right_gateway;
  o[--b] = right_gateway[1];
  o[a++] = (void *)1;
  init_next_arm(t, a, b, o, s);
...
#define Leftleft_Gate(Yellow)                                                  \
  N(leftleft_gate_##Yellow) {                                                  \
    o[--b] = ygrb_gateway;                                                     \
    o[--b] = &left_arms;                                                       \
    o[--b] = Tab_##Yellow;                                                     \
    (left_arms.arms[left_arms.i] + Yellow##_off)(t, a, b, o, s);               \
  }
Leftleft_Gate(Yellow) Leftleft_Gate(Green) Leftleft_Gate(Red)
    Leftleft_Gate(Blue);

N(locked_leftleft_gate_Yellow) {
  ygrb_separator = leftleft_gateway;
  o[--b] = leftleft_gateway[0];
  o[a++] = (void *)1;
  init_next_arm(t, a, b, o, s);
}
N(locked_leftleft_gate_Green) {
  ygrb_separator = leftleft_gateway;
  o[--b] = leftleft_gateway[1];
  o[a++] = (void *)1;
  init_next_arm(t, a, b, o, s);
}
...
G(Purple) {
  ...
  leftleft_gateway[0] = leftleft_gate_Yellow;
  leftleft_gateway[1] = leftleft_gate_Green;
  leftleft_gateway[2] = leftleft_gate_Red;
  leftleft_gateway[3] = leftleft_gate_Blue;
  ygrb_gateway[0] = yellow;
  ygrb_gateway[1] = green;
  ygrb_gateway[2] = red;
  ygrb_gateway[3] = blue;
  ...
  if (CMP(name, __FILE__) == 0 && terminals_count == 0) {
    ...
    left_separator        = locked_left_gateway;
  }
