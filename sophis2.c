...
#define N(args)                                                                \
  void args(long *ο, long β, long α, long τ, long σ, long ρ, long δ, long ι)
...
#define bo(...)                                                                \
  ({                                                                           \
    long v[] = {__VA_ARGS__}, len = sizeof(v) / sizeof(*v);                    \
    while (len)                                                                \
      ο[--ρ] = v[--len];                                                       \
  })
...
N(programS) {
  ...
  οBlue(Blue, "baaatttaaattaatataaa", 20, 0), Green_book(EE);
}
...
N(Red_book    );
N(Purple_book );
N(Purple_book_e) {τ += 11, Purple_book(EE); }
N(Navy_book   );
N(Navy_book_n) {τ -= 1 << 8,Navy_book(EE);}
N(Yellow_31   );
N(Purple_31   );
N(Navy_3      ) { (ΔLime(1) == ο[τ + 1] ? Green       : Navy_book_n)(EE); }
...
N(search      ) { τ = σ, Navy_book_n(EE); }
N(re_tau      ) { τ = ΔLime(2), Lime(EE); }
N(Navy_name   ) { οNavy(Navy, τ), Blue_book(EE); }
N(Purple_31   ) { οLime(Green_book, ο[τ+1],τ), οMaroon(clr_Red), οOlive(re_tau); οGreen(Red_book),  search(EE); }
N(Yellow_31   ) { οLime(Green_book, ο[τ+1],τ), οMaroon(clr    ), οOlive(re_tau); οGreen(Navy_name), search(EE); }
N(Green_3     ) { οLime(Green_book, ο[τ+1],τ), οMaroon(clr_top), οOlive(re_tau); οGreen(Navy_name), search(EE); }
N(Yellow_3    ) { ι = α, Maroonι(EE); }
N(Purple_3    ) { ι = α, Maroonι(EE); }
N(cursor      ) { τ = ΔGreen(1),
                  α = ΔGreen(2),
                  ρ = ΔGreen(3), Green(EE); }
N(Blue_1      ) { οGreen(Blue_book,  τ,         α, ρ), οYellow(cursor); Yellow_book(EE); }
N(Red_1       ) { οGreen(Red_book,   τ,         α, ρ), οYellow(cursor); Purple_book_e(EE); }
N(Green_4     ) { οGreen(Green_book, τ,         α, ρ), οYellow(cursor);
                  οMaroon(clr_top_Red);
                  οGreen(Red_book,   ΔNavy(1),  α, ρ), οYellow(cursor); Olive(EE); }
N(Green_5     ) { long bo = β;
...
N(Green_2     ) { const char *t = ο[τ + 1];
                  ...
                  if (pos < length && s[pos] == t[0])
                    ...
                    Yellow(EE); }
//N(Green_2     ) { οRed(Red, ο[τ+1]), Green_book(EE); }
N(Yellow_book) {
  static n_t narg[5] = {
    [2] = Green_2,
    [3] = Yellow_3,
