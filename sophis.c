...
S(beginning_Navy    ) { beginning_Lime(o); }
S(beginning_Blue    ) {}
S(beginning_Green   ) {}
S(beginning_Red     ) {}
S(beginning_Yellow  ) {}
...
S(tword_Red     ) {}
S(tword_Yellow  ) { static N((*sopcodes[])) = {tword_init, tword_pith}; sopcodes[o[τ+5]](o); }
...
S(term_Red      ) { got(o); }
S(term_Yellow   ) { const char*s = o[--α];
                    if(*s && strcmp(s, o[τ + 1]) == 0) {
                      s += strlen(o[τ + 1]);
                      o[α++] = s;
                    } else ρ -= 1;
                    got(o);
                  }
S(term_Purple   ) { o[σ] = o[τ]; o[σ+1] = o[τ+1]; σ += 11; got(o); }
...
S(put_Lime      ) { got(o); }
S(put_Navy      ) { got(o); }
S(put_Blue      ) { put_Yellow(o); }
S(put_Green     ) { put_Yellow(o); }
S(put_Red       ) { put_Yellow(o); }
S(put_Yellow    ) { o[α++] = o[τ+1]; got(o); }
...
S(dot_Maroon    ) {}
S(dot_Lime      ) {}
S(dot_Navy      ) { dot_Yellow(o); }
S(dot_Blue      ) { dot_Yellow(o); }
S(dot_Green     ) { dot_Yellow(o); }
S(dot_Red       ) { dot_Yellow(o); }
...
S(print_Fuchsia ) {}
S(print_Olive   ) { got(o); }
S(print_Maroon  ) { got(o); }
S(print_Lime    ) { got(o); }
S(print_Navy    ) { got(o); }
S(print_Blue    ) { print_Yellow(o); }
S(print_Green   ) { print_Yellow(o); }
S(print_Red     ) { print_Yellow(o); }
S(print_Yellow  ) { for(long i = 0; i < α; i++)
                      printf("%s ", (char*)o[i]);
                    α = 0;
                    printf("\n");
                    got(o); }
...
N(parse_sample) {
  long π = 0;
  σ -= 5;
  T(beginning) Ta(put, "b") Ta(term, "a") T(print)            T(dot)AL
  σ += 5;
  got(o);
}
